...
I used Claude throughout this project - notable usage was to explain SQLAlchemy relationship syntax I wasn't confident reading on my own, to build a small debugging tool (the debug service).

**Understanding `db.relationship` in models.py.** I hadn't worked with SQLAlchemy's relationship/association-table pattern before, so I asked it to walk through `models.py` line by line — specifically what `secondary=playlist_entries` on `Playlist.songs` means (it's the many-to-many join table, and songs come back through it rather than through a foreign key on `Song` itself), why `User.friends` needs both `primaryjoin` and `secondaryjoin` instead of just `secondary` (because `friendships` joins the `User` table to itself, so SQLAlchemy can't infer which side of the join is "me" vs. "my friend" without being told explicitly), and what `lazy="subquery"` on `Song.tags` actually does at query time (tags are fetched via a second, separate query rather than as part of the same join). That last explanation turned out to matter directly for Issue #3 — it's the reason I could confidently say the `.outerjoin(song_tags, ...)` in `search_songs` was dead code rather than assume it was doing something for tag loading.

**Building the debug harness (`services/debug_service.py` + `routes/debugging.py`).** I wanted a way to reproduce the Sunday-streak bug (Issue #1) through curl instead of waiting for an actual Sunday or writing a one-off script every time. My first ask was open-ended ("let's make a debugging controller that fakes data"), and its first proposal modified `record_listening_event`'s signature in `streak_service.py` to accept a fake `now` — which I rejected, since I didn't want any existing service file touched. I pushed back and asked for a version that only adds new services and reuses existing functions as-is. It then designed `debug_service.simulate_listen`, which creates the supporting `User`/`ListeningEvent` rows itself but calls the existing `update_listening_streak(user, now)` unmodified (that function already took an explicit `now` argument, so it didn't need to be touched at all — something I hadn't noticed until it pointed it out). I didn't just trust that this worked: I ran the endpoint myself against an in-memory DB for a Saturday→Sunday pair and confirmed the streak stayed at 1 instead of advancing to 2, matching what `pytest` independently showed for the same bug.

**Where I had to verify it myself / it pointed me in a wrong direction.** For Issue #3, its first theory — and mine, since the code pattern is a well-known one — was that `search_songs`'s join against `song_tags` would produce duplicate rows for any song with more than one tag, because a join fans out one row per match. That's a correct description of what the join does at the SQL level, and I initially treated it as the full explanation. But when I actually ran `pytest tests/test_search.py`, the test built specifically to catch this (a song with three tags, asserting exactly one result) **passed** — no duplicates. Rather than accepting the theoretical explanation, I checked the raw SQL directly against the database (bypassing the ORM) and confirmed the join really does return three physical rows, but SQLAlchemy 2.0's `Query(Song).all()` deduplicates full-entity results by primary key before handing them back — a version-specific ORM behavior neither of us had accounted for up front. So the join is still a genuine defect worth fixing (it's dead code that produces row fan-out for no reason), but I had to correct the initial claim that it was *currently* producing visible duplicate search results, and documented that distinction explicitly in the Issue #3 entry below rather than writing up a repro I hadn't actually observed.
